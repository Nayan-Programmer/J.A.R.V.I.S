...
function handleBackgroundTasks(tasks, contentEl) {
    ...
    tasks.forEach(task => {
        ...
        const _lmap = {
            ...
            'generate_ppt_email': 'Presentation + Email',
            'content': 'Content Writing',
            'track_phone': 'Phone Lookup',
        };
        const label = _lmap[task.type] || task.type;
...
function updateTaskCard(cardEl, status, data) {
    ...
    if (status === 'completed') {
        ...
        // ── PPT: download button ───────────────────────────────────────
        if (result && result.type === 'ppt' && result.download_url) {
            const wrap = document.createElement('div');
            wrap.style.cssText = 'margin-top:10px;display:flex;flex-direction:column;gap:6px;';
            const dlBtn = document.createElement('button');
            dlBtn.className = 'bg-task-view-btn';
            dlBtn.textContent = 'Download Presentation (.pptx)';
            ...
            if (result.emailed) {
                ...
                note.style.cssText = 'font-size:0.85em;color:#7cfc9a;margin-top:2px;';
                note.textContent = 'Email sent successfully.';
                wrap.appendChild(note);
            } else if (result.email_message) {
                const note = document.createElement('div');
                note.style.cssText = 'font-size:0.82em;color:#f4a261;margin-top:2px;';
                note.textContent = result.email_message;
                wrap.appendChild(note);
            }
        ...
        // ── Phone: info panel ─────────────────────────────────────────
        if (result && result.type === 'phone' && result.text) {
            const wrap = document.createElement('div');
            wrap.style.cssText = 'margin-top:10px;background:rgba(255,255,255,0.05);border-radius:8px;padding:10px 14px;font-size:0.92em;line-height:1.7;white-space:pre-wrap;';
        ...
        if (result && result.type === 'image' && result.url) {
            const imgWrap = document.createElement('div');
            imgWrap.style.cssText = 'margin-top:10px;';
            const img = document.createElement('img');
            img.src = result.url;
            img.alt = result.prompt || 'Generated image';
            img.style.cssText = 'max-width:100%;max-height:340px;border-radius:10px;display:block;box-shadow:0 2px 12px rgba(0,0,0,0.25);cursor:pointer;';
        ...
        viewBtn.textContent = (result && result.type === 'image') ? 'Open full image' : 'Open in new tab';
        ...
        // Auto-open viewer tab only for content; images show inline
        if (!result || result.type !== 'image') {
            try {
                const taskId = cardEl.dataset.taskId;
                const w = window.open(`${window.location.origin}/app/viewer.html?task_id=${taskId}`, '_blank');
                if (!w) showToast('Result ready! Click "Open in new tab" to view.');
            } catch (_) {}
        }
